<?php
// created by duangan, 2015-5-4 -->
// support topic deal function.    -->

// 朝代 big
$topic_big = array(
    "历史",
    "科学技术",
    "人文社会",
    "宗教",
    "西方思想",
    "中国思想",
    "经济",
    "艺术体育",
    "其它"
);

// 朝代 tag vip struct.
$topic = array
(
    array
    (
        // 历史
        array("历史学", "super", "multe-key", "历史研究", "史学"),
        array("分子遗传学", "super", "multe-key", "基因", "克隆"),
        array("语言学", "super", "multe-key", "汉藏语系", "印欧语系", "阿尔泰语系", "闪含语系", 
                "达罗毗荼语系", "高加索语系", "乌拉尔语系"),
        array("地理", "super", "multe-key", "地球科学", "地质学", "气候学"),
        array("交通", "super", "multe-key", "国道", "直道", "水路", "航线", "高速公路",
            "海路", "海运", "河运", "运河", "大道", "驿站", "驿马"),
        array("地质年代"),
    ),
    array
    (
        // 科学技术
        array("科学", "super"),
        array("发明创造", "super"),
        array("天文学", "super", "multe-key", "航天", "太阳", "月球", "恒星", "行星", "卫星", "火星", 
                "月食", "日食", "黑洞", "星图", "历法", "水星", "土星", "水星", "天王星", "海王星", 
                "冥王星", "彗星", "星云", "银河", "星系", "宇宙", "陨石", "航天飞机", "宇宙飞船", 
                "空间站", "望远镜", "NASA"),
        array("物理学", "super"),
        array("航空"),
        array("生物学", "super", "multe-key", "植物学", "动物学", "生物化学"),
        array("生态学", "super", "multe-key", "环境保护", "环保"),
        array("化学"),
        array("数学", "normal", "multe-key", "代数", "几何", "逻辑学", "数论", "拓扑", "微积分", 
            "方程", "概率论", "统计学"),
        array("医学"),
        array("诺贝尔奖", "super", "sigle-key", "诺贝尔奖"),
    ),
    array
    (
        // 人文社会
        array("社会", "super", "sigle-key", "社会"),
        array("人口", "super", "sigle-key", "人口"),
        array("教育", "super", "sigle-key", "教育"),
        array("伦理学"),
        array("心理学"),
    ),
    array
    (
        // 宗教
        array("宗教", "super", "multe-key", "耆那教", "拜火教", "祆教", "摩尼教", "景教"),
        array("基督教", "super", "multe-key", "耶稣会", "多明我会", "共济会", "圣公会", "方济各会", "循道公会",
                "循道会", "教宗", "教皇", "布道会", "浸信会", "圣经会", "公理会", "长老会", "归正会", "卫理公会", 
                "殉道", "巴色会", "联合路德会", "圣经", "新约", "旧约", "基督", "耶稣", "福音", "基督教", "新教",
                "东正教", "天主教", "修女", "修士", "传教士", "受洗", "浸礼会"),
        array("东正教"),
        array("犹太教"),
        array("伊斯兰教", "super", "multe-key", "古兰经", "斋月"),
        array("佛教"),
        array("道教"),
        array("印度教"),
        array("萨满教"),
    ),
    array
    (    
        // 西方思想
        array("古希腊哲学", "super", "multe-key", "希腊哲学", "普罗泰戈拉", "德谟克利特", "芝诺", "苏格拉底", 
            "柏拉图", "亚里士多德", "伊壁鸠鲁", "泰勒斯", "米利都学派"),
        array("经院哲学"),
        array("康德哲学", "super", "multe-key", "康德"),
        array("经验主义"),
        array("存在主义"),
        array("后现代哲学"),
        array("文艺复兴"),
        array("启蒙运动"),
        array("马克思主义", "normal", "multe-key", "马列主义", "马克思", "恩格斯", "列宁", "斯大林"),
    ),
    array
    (
        // 中国思想
        array("中国思想家", "super", "key-time", "阴阳家", "杂家", "纵横家", "邹衍", 
                "张仪", "苏秦", "吕不韦", "吕氏春秋", "淮南子", "许行", "孙武", "司马穣苴", "孙膑", 
                "吴起", "尉缭", "魏无忌", "白起", "司马法", "-500", "200"),
        array("文字狱", "super", "multe-key", "焚书", "坑儒", "禁书"),
        array("儒家", "super", "multe-key", "孔子", "孟子", "四书五经", "论语", "尚书", "中庸", 
                "周易", "荀子", "董仲舒", "程颐", "朱熹", "陆九渊", "王阳明", "朱程理学", "心学"),
        array("道家", "super", "multe-key", "老子", "庄子", "列子", "道德经", "慎到", "杨朱"),
        array("法家", "super", "key-time", "管子", "韩非子", "商鞅", "商君书", "李斯", "吴起", 
                "-500", "200"),
        array("墨家", "super", "multe-key", "墨子"),
        array("名家", "super", "multe-key", "邓析", "公孙龙", "惠子", "惠施", "杨子"),
    ),
    array
    (
        // 经济
        array("经济", "super", "sigle-key", "经济"),
        array("经济危机", "super", "multe-key", "金融危机", "股灾"),
        array("农业"),
        
        array("工业"),
        array("金融业"),
        array("IT技术"),
        array("IT行业"),
        array("手机行业"),
        array("能源", "super", "multe-key", "石油", "煤", "天然气", "太阳能", "核电", "可再生能源", 
            "油页岩", "水电", "火电", "风电", "碳排放", "温室效应"),
    ),
    array
    (
        // 艺术文化体育
        array("文学", "super", "sigle-key", "诗", "诗歌", "散文", "剧本", "童话", "寓言",
            "象征主义", "现实主义", "浪漫主义", "自然主义", "乐府", "格律", "歌谣", "话本", "骈文", 
            "对仗", "平仄"),
        array("小说"),
        array("音乐", "super", "sigle-key", "民族乐器", "西洋乐器", "钢琴", "竖琴", "提琴", 
            "口琴", "电子琴", "管乐器", "弦乐器", "打击乐器", "锣", "鼓", "钹", "吉他", "琵琶", "笛", 
            "二胡", "编钟", "贝司", "萨克斯", "交响乐", "乐队", "乐团", "民乐", "古筝"),
        array("美术"),
        array("雕像"),
        array("电影"),
        array("电视剧"),
        array("书法"),
        array("体育"),
        array("奥运会", "super", "sigle-key", "奥林匹克"),
        array("奥斯卡奖", "super", "sigle-key", "小金人"),
    ),
    array
    (
        // other
    )
);

?>