...
class my_insert {
	...
	{
		$sql = "insert into web_page (page_url_md5, page_url, page_content, page_length, grab_time) 
		   values ('".md5($url)."','$url','$content',$filesize,'".date("Y-m-d H:i:s", time())."')";
	...
	function put_web_pic($url, $content, $filename, $filesize, $ref_page) {
		$sql = "insert into pic_gallery (file_url_md5, file_url, file_data, file_name, file_size) 
		      values ('".md5($url)."','$url','$content','$filename',$filesize)";
		$result = mysql_query($sql);
		$id = mysql_insert_id();
		return $id;
	}
