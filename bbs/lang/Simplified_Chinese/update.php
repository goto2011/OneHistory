...
'Update'						=>	'更新BBS',
'Update message'				=>	'您的BBS数据库已经过时，继续之前必须升级。如果您是该论坛管理员，请按下列说明完成升级。',
'Note'							=>	'注意：',
'Members message'				=>	'该进程仅供论坛管理员操作。如您不是管理员，请勿担心——论坛马上就会上线！',
'Administrator only'			=>	'该步骤仅供论坛管理员操作！',
'Database password info'		=>	'要执行数据库升级请填写BBS所使用的数据库密码。如果您不慎忘记了密码，请查看 \'config.php\' 文件中储存的密码。',
...
'You are running error'			=>	'您所运行的 %1$s 版本号为 %2$s。BBS %3$s 至少必须 %1$s %4$s 才能正确运行。继续之前您必须先将您的 %1$s 升级。',
'Version mismatch error'		=>	'版本不匹配。数据库 \'%s\' 似乎不包含本升级脚本所支持的BBS数据库架构。',
'Invalid file error'			=>	'无效的数据库名称。当使用SQLite数据库时，填写的数据库名称必须要与您的 \'%s\' 文件中显示的一致',
'Invalid password error'		=>	'无效的数据库密码。升级 BBS 时您填写的数据库密码必须要与您的 \'%s\' 文件中显示的一致',
...
'No charset conversion'			=>	'<strong>重要！</strong> BBS 已经检测到该PHP环境尚不支持将ISO-8859-1以外的编码转换为UTF-8的编码转换机制。这意味着如果您的当前编码不是ISO-8859-1，BBS 将不能自动将您的论坛数据转换为UTF-8格式，您必须手工完成该操作。关于手工转换字符集的指示可在更新说明中找到。',
'Enable conversion'				=>	'<strong>启用转换：</strong> 如启用该功能，在更新脚本完成数据库结构的必须更新后，还会将数据库中的所有文字由当前编码转换为UTF-8。如是从BBS 1.2版更新，该转换过程是必须的。',
...
'Error info 1'					=>	'转换某些用户时出现错误。当从BBS 1.2转换时，如果多个用户的名称过于相似，如 "bob" 和 "böb"，可能会出现该错误。',
