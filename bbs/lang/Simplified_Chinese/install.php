...
'Next'							=>	'下一步',
'Already installed'				=>	'BBS 似乎已安装。请点击<a href="index.php">此链接</a>前往。',
'You are running error'			=>	'您的 %1$s 版本为 %2$s。 BBS %3$s 至少需要 %1$s %4$s 方可正确运行。您在继续之前，必须升级您的 %1$s 设置。',
'My BBS Forum'				=>	'我的BBS论坛',
'Description'					=>	'BBS是什么？需要您用心体会。',
...
'No DB extensions'				=>	'现有 PHP 环境不具备 BBS 所需的任何格式的数据库。为使 BBS 能被安装，PHP 必须已包含 MySQL、PostgreSQL或SQLite。',
...
'BBS Installation'			=>	'BBS 安装向导',
'Welcome'						=>	'您将要安装 BBS。继续安装前您必须完成填写以下表格。遇到任何安装问题，请参考帮助文档。',
'Install'						=>	'安装 BBS %s',
'Errors'						=>	'下列错误须要修正：',
'Database setup'				=>	'数据库设置',
'Info 1'						=>	'用于连接您的数据库的所有信息。',
'Select database'				=>	'选择数据库类型',
'Info 2'						=>	'BBS 目前支持 MySQL， PostgreSQL 以及 SQLite。',
'Database type'					=>	'数据库类型',
'Required'						=>	'(必需)',
...
'Info 6'						=>	'如果您需要在同一个数据库安装多个BBS论坛，请修改此项。',
'Table prefix'					=>	'数据表前缀',
...
'Existing table error'			=>	'数据库 \'%s\'中已经存在数据表 \'%susers\'。这可能表示 BBS 已曾安装或另一个已安装的软件占用了 BBS 所需的一个或多个数据表名称。如您准备在同一数据库中安装多个 BBS 副本，您必须选择一个不同的表前缀。',
...
'Test post'						=>	'示例帖子',
'Message'						=>	'如果您在阅读这个帖子(我猜正是如此)，安装的 BBS 已经开始工作！现在您可以登录，并进入顶部的管理设置面板来配置您的论坛。',
'Test category'					=>	'测试分类',
'Test forum'					=>	'测试版块',
'This is just a test forum'		=>	'这只是一个测试版块',
'Alert cache'					=>	'<strong>缓存所在目录不可写！</strong>为使 BBS 功能正常，必须为目录 <em>%s</em> 开启 PHP 写入权限。使用 chmod 命令设置恰当的目录权限。如有疑问，用 chmod 设为0777。',
...
'BBS has been installed'		=>	'BBS 已安装结束。请遵循以下指示完成安装向导。',
'Final instructions'			=>	'最后一步',
'Info 17'						=>	'为完成安装，您需要点击下示按钮以下载名为 config.php 的文件。然后上传它至您已安装的 BBS 程序的根目录下。',
'Info 18'						=>	'一旦您上传完 config.php，BBS 就安装成功了！随后您便可 <a href="index.php">进入论坛首页</a>。',
'Download config.php file'		=>	'下载 config.php 文件',
'BBS fully installed'		=>	'BBS 已经安装成功！您现在可 <a href="index.php">点此访问论坛首页</a>。',
