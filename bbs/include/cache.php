...
function generate_config_cache()
{
	...
	bbs_write_cache_file('cache_config.php', $content);
}
...
function generate_bans_cache()
{
	...
	bbs_write_cache_file('cache_bans.php', $content);
}
...
function generate_quickjump_cache($group_id = false)
{
	...
	foreach ($groups as $group_id => $read_board)
	{
		...
		bbs_write_cache_file('cache_quickjump_'.$group_id.'.php', $output);
	}
}
...
function generate_censoring_cache()
{
	...
	bbs_write_cache_file('cache_censoring.php', $content);
}
...
function generate_stopwords_cache()
{
	...
	bbs_write_cache_file('cache_stopwords.php', $content);
}
...
function generate_users_info_cache()
{
	...
	bbs_write_cache_file('cache_users_info.php', $content);
}
...
function generate_admins_cache()
{
	...
	bbs_write_cache_file('cache_admins.php', $content);
}
...
//
function bbs_write_cache_file($file, $content)
{
	...
	bbs_invalidate_cached_file(FORUM_CACHE_DIR.$file);
}
...
function clear_feed_cache()
{
	...
	while (($entry = $d->read()) !== false)
	{
		if (substr($entry, 0, 10) == 'cache_feed' && substr($entry, -4) == '.php')
			...
			bbs_invalidate_cached_file(FORUM_CACHE_DIR.$entry);
	}
	$d->close();
}
...
//
function bbs_invalidate_cached_file($file)
{
